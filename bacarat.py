...
# Equipe: Rodrigo Eiji Furukawa e Pedro Dannecker
# Data: 17/10/2020
...
while repeat:
    ...
    while i < numjogadores:
        if resultado == apostado[i] and apostado[i] == 'a' :
            if baralhos == 1:
                ...
                fichas[i] = fichas[i] + int(aposta[i] - aposta[i]* 0.0129)
            if baralhos == 6 or baralhos == 8:
                ...
                fichas[i] = fichas[i] + int(aposta[i] - aposta[i]* 0.0124)
        ...
        if resultado == apostado[i] and apostado[i] == 'b':
            if baralhos == 1:
                ...
                fichas[i] = int( int(aposta[i]*0.95) - aposta[i]*0.95 * 0.0101  )
            if baralhos == 6 or baralhos == 8:
                ...
                fichas[i] = int( int(aposta[i]*0.95) - aposta[i]*0.95 * 0.0106  )
        ...
        if resultado == apostado[i] and apostado[i] == 'c':
            if baralhos ==1 :
                ...
                fichas[i] = int(aposta[i] * 8 - (aposta[i] * 8)* 0.1575)  + fichas[i]
            if baralhos ==6 :
                print('Parabéns! O jogador {1} ganhou . Seu novo saldo é de {0} fichas.'.format(int(aposta[i] * 8 - (aposta[i] * 8)* 0.1444)  + fichas[i], i))
                fichas[i] = int(aposta[i] * 8 - (aposta[i] * 8)* 0.1444)  + fichas[i]
            if baralhos ==8 :
                print('Parabéns! O jogador {1} ganhou . Seu novo saldo é de {0} fichas.'.format(int(aposta[i] * 8 - (aposta[i] * 8)* 0.1436)  + fichas[i], i))  
                fichas[i] =  int(aposta[i] * 8 - (aposta[i] * 8)* 0.1436)  + fichas[i]
        
        i+=1
    #saber se algum jogador ainda tem fichas
    fichastotal = sum(fichas)
    if fichastotal > 0: 
        querjogar = input('Você deseja continuar jogando ? sim ou nao : ')
        if querjogar == 'sim':
            repeat = True
        if querjogar == 'nao':
            repeat = False
    ...
    if fichastotal == 0:
        repeat =  False
        print("Todos os jogadores zeraram suas fichas, voces perderam")
